...
def main():
    ...
    # client_idx= -1 to trun off the channel selection 
    ...
    # weight_list = set(nets[0].state_dict().keys())
    # for i in nets[1:]:
    #     client_weights_dict = i.state_dict()
    #     weight_list.intersection_update(client_weights_dict.keys())
    # print("Common weights: ", weight_list)
    # exit()


    # Init HyperCrossAttention
    hyper = HyperCrossAttention_embedding_res(nets, K=args.num_nets, init_beta=0.1, mode="both", site_texts=SITES_TEXT)
    ...
    for com_round in range(args.com_rounds):
        ...
        for i in range(args.num_nets):
            ...
            for local_epoch in range(args.local_epochs):
                ...
                global_model = last_param_dict_list[i] if local_epoch != 0 else None
                if current_task == 'dep':
                    loss, prompt_loss, non_prompt_loss = function_depth.train_sam(args, current_net, current_optimizer1, current_optimizer2, current_nice_train_loader, local_epoch,global_model = global_model)
                else:
                    loss, prompt_loss, non_prompt_loss = function.train_sam(args, current_net, current_optimizer1, current_optimizer2, current_nice_train_loader, local_epoch, global_model = global_model)
            ...
            if current_task == 'dep':
                ...
                logger.info(f'Dataset: {current_dataset} || Task: {current_task} || Total score: {tol}, RMSE: {eiou}, RMSE: {edice} || @ Communication rounds {com_round}.')
            else:
            ...
            if current_task == 'dep':
                if eiou < best_mse_list[i]:
                    ...
                    # vis the depth map
                    function_depth.vis_sam(args, current_nice_test_loader, com_round, current_net, visual_path, writer,vis=False)
                ...
                if edice > best_dice_list[i]:
                    ...
                    function.vis_sam(args, current_nice_test_loader, com_round, current_net, visual_path, writer,vis=False)
                    torch.save(current_net.state_dict(), checkpoint_path.format(net=i, epoch=com_round, type='best_dice'))

        
        # # Aggregation and update
        last_param_dict_list = FedAvg(weights_dict_list_not_update, num_clients = args.num_nets, Tasks=TASKS,  Layers=args.Layers, client_weights=WEIGHTS_CLIENTS, Task_specific=args.Task_specific)
