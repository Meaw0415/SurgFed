...
def main():
    ...
    net = get_network(args, args.net, use_gpu=args.gpu, gpu_device=GPUdevice, distribution = args.distributed,if_depth=True)
    ...
    for epoch in range(settings.EPOCH):
        ...
        loss, prompt_loss, non_prompt_loss = function_depth.train_sam(args, net, optimizer1, optimizer2, nice_train_loader, epoch)
        ...
        if epoch % args.val_freq == 0 or epoch == settings.EPOCH-1:
            tol, (eiou, edice) = function_depth.validation_sam(args, nice_test_loader, epoch, net, writer)
            ...
            if tol < best_tol:
                ...
                function_depth.vis_sam(args, nice_test_loader, 0, net, '/mnt/iMVR/zhengf/Medical-SAM2/vis/dep/bb', None,vis=True)
                logger.info(f'New best model saved at epoch {epoch} with total score: {tol}.')
